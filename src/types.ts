...
export interface Transport {
  ...
  /** PUT raw bytes to an absolute (presigned) URL, when supported by the transport. Used by file uploads. */
...
/** Vector record id. Strings and integers are both accepted; integers serialize as JSON numbers. */
export type VectorRecordId = string | number;

/** Vector record to insert. */
export interface VectorRecord {
  /** Optional record id; generated by the API when omitted. Accepts a string or an integer (preserved as a JSON number). */
  id?: VectorRecordId;
  /** Vector values. (`vector` is accepted as a legacy alias and normalized to `values`.) */
  values: number[];
...
export interface AddTextsRequest {
  ...
  texts: Array<string | { id?: VectorRecordId; text: string; metadata?: JsonObject }>;
...
/**
 * Ingest-source input: an existing source id string, a reference to an existing
 * source, or a source builder/options payload (e.g. from `webSource(...)`) that
 * the SDK creates before starting the job.
 */
export type IngestSourceInput = string | IngestSourceRequest | IngestionSourceInput;

/** Options for {@link DatasetsClient.ingestSource}. */
export interface IngestSourceOptions {
  /** Pipeline id to run. Omit to use the default ingestion pipeline. */
  pipelineId?: string;
}

/** Options for {@link DatasetsClient.ingestFiles}. */
export interface IngestFilesOptions {
  /** Name for the auto-created `file_upload` source. Defaults to `file-upload-<basename>`. */
  sourceName?: string;
  /** Description for the auto-created source. */
  description?: string;
  /** Metadata merged into the auto-created source metadata. `dataset_id` is always included. */
  metadata?: JsonObject;
}

/** Options for {@link DatasetsClient.ingestFilesystem} disk walking. */
export interface IngestFilesystemOptions {
  /** Metadata merged into the auto-created source metadata. */
  metadata?: JsonObject;
  /** File extensions to include. Defaults to common text extensions such as `.txt`, `.md`, `.json`, and `.csv`. */
  extensions?: string[];
  /** Maximum file size to include. Defaults to 1 MiB. */
  maxBytesPerFile?: number;
  /** Name for the auto-created `file_upload` source. Defaults to `file-upload-<basename(root)>`. */
  sourceName?: string;
}

/** One presigned upload target returned by `upload/init`. */
export interface UploadTarget {
  /** Server-assigned file id, echoed back on `upload/complete`. */
  fileId?: string;
  /** Server-assigned file id in snake_case. */
  file_id?: string;
  /** Presigned URL to `PUT` the file bytes to. */
  uploadUrl?: string;
  /** Presigned URL in snake_case. */
  upload_url?: string;
  /** Additional API fields. */
  [key: string]: unknown;
}

/** Response returned by `POST /ingestion/sources/{id}/upload/init`. */
export interface InitUploadResponse {
  /** Upload job id, passed to `upload/complete`. */
  jobId?: string;
  /** Upload job id in snake_case. */
  job_id?: string;
  /** Presigned upload targets, one per requested file. */
